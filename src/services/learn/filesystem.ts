...
export function GetTracks(): Array<Track> {
    ...
    const dirs = fs.readdirSync(dir, { withFileTypes: true })
        .filter((i) => i.isDirectory() && fs.readdirSync(join(dir, i.name), { withFileTypes: true })
            .some((i) => i.isFile() && i.name.endsWith('.md')))
    ...
    return dirs.map((i) => {
        ...
        const lessons = fs.readdirSync(trackDir, { withFileTypes: true })
