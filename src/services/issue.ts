...
export async function GetRepos(): Promise<Repository[]> {
    ...
    while (currentPage) {
        ...
        repos.push(...body.items.map((i: any) => {
            return {
                id: i.id,
                name: i.name,
...
export async function GetIssues(): Promise<Issue[]> {
    ...
    while (currentPage) {
        ...
        const body = await response.json()
        issues.push(...body.items.map((i: any) => {
            return {
                id: i.id,
                number: i.number,
                title: i.title,
                body: i.body,
                url: i.html_url,
                labels: i.labels.map((l: any) => { return { name: l.name, color: l.color } }),
                author: {
                    id: i.user.id,
                    login: i.user.login,
                    avatarUrl: i.user.avatar_url ?? 'https://camo.githubusercontent.',
                    url: i.user.html_url,
                },
